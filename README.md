...
> ```
>
> This bites any config that shells out to `hyprctl` — `hypridle.conf` in particular.
...
Enabled session services (they rely on the target above, and would silently never
start without it):
